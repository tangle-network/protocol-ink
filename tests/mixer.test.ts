...
describe('mixer', () => {
    ...
    it('Creates a new instance of the mixer', async () => {
        ...
        const poseidonABI = await artifacts.readArtifact('poseidon');
        console.log();
        ...
        const mixerVerifierABI = await artifacts.readArtifact('mixer_verifier');
        ...
        const mixerContract = await mixerContractFactory.deploy('new',
            levels,
            depositSize,
            poseidonABI.source.hash,
            mixerVerifierABI.source.hash,
        );
        const mixerABI = artifacts.readArtifact('mixer');
        console.log(mixerContract);
    });
})